...
class BHyveClient:
    ...
    async def _refresh_device_history(
        ...
        now = time.time()
        last_poll = self._last_poll_device_histories.get(device_id, 0)
        if force_update:
            _LOGGER.info("Forcing refresh of device history %s", device_id)
        elif now - last_poll < API_POLL_PERIOD:
            return
    ...
    async def _refresh_landscapes(
        ...
        now = time.time()
        last_poll = self._last_poll_landscapes.get(device_id, 0)
        if force_update:
            _LOGGER.debug("Forcing landscape refresh %s", device_id)
        elif now - last_poll < API_POLL_PERIOD:
            return

        self._landscapes[device_id] = await self._request(
            "get",
